...
import static org.example.Iterators.generate;
...
import static org.example.Iterators.toList;
import static org.example.Iterators.zip;
...
public class MonteCarloTest {
    public static void piDemo() {
        ...
        BiFunction<Long, Supplier<Integer>, Double> monteCarlo =
                (n, experiment) -> Mathx.sum(limit(generate(experiment), n)) / n;

        /*
         * 몬테카를로 방식으로 값을 어림잡을 수 있습니다. 두 마구잡이 수가 서로 소인지 알아보는 함수 Mathx.dirichletTest를 씁니다.
         */

        System.out.println(Math.sqrt(6.0 / monteCarlo.apply(10000L, () -> Mathx.dirichletTest() ? 1 : 0)));

        /*
         * 몬테카를로 iterator를 정의합니다. 오로지 계산 방법 그 자체만을 있는 그대로 표현한 코드를 쓸 수 있습니다.
         * 계산 시간(횟수)와 계산 공간 문제 곧 계산 자원 문제를 계산 방법으로부터 격리할 수 있습니다.
         */

        // TODO: Iterators.{iterate, zip}을 써서 코드 채우기

        Function<Supplier<Integer>, UndefinedIterator<Double>> monteCarloIterator =
                (experiments) -> zip((sum, count) -> sum.doubleValue() / count,
                        iterate(0, x -> Math.addExact(x, experiments.get())), iterate(0, x -> Math.addExact(x, 1)));

        UndefinedIterator<Double> pi =
                map(monteCarloIterator.apply(() -> Mathx.dirichletTest() ? 1 : 0), ratio -> Math.sqrt(6.0 / ratio));
        System.out.println(get(pi, 1_000_000));
    }
    ...
    private static void potionTestWithInfiniteIterators() {
        ...
        UndefinedIterator<Integer> herbAvailablities = Mathx.binaryDistribution(herbRatio);
        ...
        UndefinedIterator<Quality> herbQualities =
                zip(((available, effect) -> available == 1 ? Quality.BEST : Quality.values()[effect]),
        ...
        UndefinedIterator<Double> medicineEffects = map(herbQualities, quality -> {
